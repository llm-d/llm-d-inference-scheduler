...
//	        config:
//	          headroomSelectionStrategy: "least"
func PDSLOAwareScorerFactory(name string, rawConfig json.RawMessage, handle plugins.Handle) (plugins.Plugin, error) {
	// Parse base SLO aware router config
	cfg := slo_aware_router.DefaultConfig
	if len(rawConfig) > 0 {
	...
	baseRouter := slo_aware_router.NewSLOAwareRouter(cfg, predictor).WithName(name)
