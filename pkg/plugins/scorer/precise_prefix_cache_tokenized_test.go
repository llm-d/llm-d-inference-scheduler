...
type mockKVCacheIndexer struct {
	...
	getPodScoresFromTokensFunc func(ctx context.Context, tokens []uint32, modelName string, podIdentifiers []string) (map[string]float64, error)
}
...
func (m *mockKVCacheIndexer) GetPodScoresFromTokens(ctx context.Context, tokens []uint32, modelName string, podIdentifiers []string) (map[string]float64, error) {
	if m.getPodScoresFromTokensFunc != nil {
		return m.getPodScoresFromTokensFunc(ctx, tokens, modelName, podIdentifiers)
	}
	return nil, nil
}
...
func TestPrecisePrefixCacheScorer_UsesTokenizedPrompt(t *testing.T) {
	...
	scorer := &PrecisePrefixCacheScorer{
		...
		kvCacheIndexer: &mockKVCacheIndexer{
			getPodScoresFromTokensFunc: func(_ context.Context, tokens []uint32, modelName string, _ []string) (map[string]float64, error) {
...
func TestPrecisePrefixCacheScorer_SkipsTokenizedPromptWhenEmpty(t *testing.T) {
	...
	scorer := &PrecisePrefixCacheScorer{
		...
		kvEventsConfig: &kvevents.Config{},
		kvCacheIndexer: &mockKVCacheIndexer{
			getPodScoresFromTokensFunc: func(_ context.Context, _ []uint32, _ string, _ []string) (map[string]float64, error) {
	...
	scorer.Score(ctx, scheduling.NewCycleState(), request, testEndpoints)
	assert.False(t, fromTokensCalled, "GetPodScoresFromTokens should not be called with empty TokenIDs")
}
