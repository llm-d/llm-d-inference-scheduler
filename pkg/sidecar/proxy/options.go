...
func (opts *Options) AddFlags(fs *pflag.FlagSet) {
	...
	if opts.goFlagSet == nil {
		opts.goFlagSet = flag.NewFlagSet("goFlagSet", flag.ContinueOnError)
		// Add logging flags to the standard flag set
		opts.loggingOptions.BindFlags(opts.goFlagSet)
		// Add Go flags to pflag (for zap options compatibility)
		fs.AddGoFlagSet(opts.goFlagSet)
	}
	...
	fs.StringVar(&opts.inlineConfiguration, inlineConfiguration, "", "Sidecar configuration in YAML provided as inline specification. Example `--configuration={port: 8085, vllm-port: 8203}. Inline configuration takes precedence over file configuration if both are provided.`")
	fs.StringVar(&opts.fileConfiguration, configurationFile, "", "Path to file which contains sidecar configuration in YAML. Example `--configuration-file=/etc/config/sidecar-config.yaml`. File configuration is ignored if inline configuration is provided.")
}
...
func (opts *Options) extractYAMLConfiguration() error {
	...
	if err := opts.mergeYAMLConfiguration(yamlConfiguration); err != nil {
		return err
	}
	return nil
}
...
func (opts *Options) mergeYAMLConfiguration(cfg yamlConfiguration) error {
	...
	if cfg.PrefillerUseTLS != nil && !opts.isFlagSet(prefillerUseTLS) && !opts.isFlagSet(enableTLS) {
		opts.prefillerUseTLS = *cfg.PrefillerUseTLS
	}
	if cfg.DecoderUseTLS != nil && !opts.isFlagSet(decoderUseTLS) && !opts.isFlagSet(enableTLS) {
