...
// VLLMRunner implements P/D disaggregation for vLLM backends using a decode-first approach.
// It can be configured with different connector protocols via the RequestBuilderFactory:
//   - NIXLV2RequestBuilderFactory for NIXL v2 connector
//   - DefaultRequestBuilderFactory for shared-storage connector
...
// Run executes the P/D disaggregation workflow using the configured connector protocol.
func (vr *VLLMRunner) Run(w http.ResponseWriter, r *http.Request, prefillPodHostPort string, logger logr.Logger) {
	logger = logger.WithValues("connector", vr.connector.String(), "prefillHost", prefillPodHostPort)
	...
	if cacheHitThreshold, hasCacheHitThreshold := completionRequest[keys.RequestFieldCacheHitThreshold]; hasCacheHitThreshold {
		...
		needsPrefill, err := vr.tryDecode(w, decodeReq, completionRequest, logger)
		if err != nil {
	...
	requestBuilder := vr.requestBuilder.New()
	prefillRequest := requestBuilder.PreparePrefillRequest(completionRequest)

	prefillResponse, err := vr.prefill(w, r, prefillPodHostPort, prefillRequest, logger)
	if err != nil {
...
func (vr *VLLMRunner) tryDecode(w http.ResponseWriter, r *http.Request, completionRequest map[string]any, logger logr.Logger) (bool, error) {
	...
	return vr.tryDecodeBuffered(w, r, logger)
}
...
func (vr *VLLMRunner) hasCacheThresholdFinishReason(response map[string]any) bool {
...
func (vr *VLLMRunner) checkBufferedResponseForCacheThreshold(data string, logger logr.Logger) bool {
...
func (vr *VLLMRunner) prefill(w http.ResponseWriter, r *http.Request, prefillPodHostPort string, completionRequest map[string]any, logger logr.Logger) (map[string]any, error) {
