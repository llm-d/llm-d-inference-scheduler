...
Scores pods based on the number of active requests being served per pod. It consumes the
in-flight request counts produced by the `inflight-load-producer`. Pods with fewer active
...
- **Parameters**:
  - `idleThreshold`: maximum active request count for a pod to be considered idle.
  - `maxBusyScore`: maximum score assigned to pods above `idleThreshold`.
  - `requestTimeout`: deprecated/ignored; accepted only for backward compatibility and has no effect.
