...
// ConversationsHandler is a mock handler for OpenAI Conversations API
type ConversationsHandler struct {
	Connector              string
	Role                   Role
	RequestCount           atomic.Int32
	ConversationsRequests  []map[string]any
	ConversationsResponses []map[string]any
	mu                     sync.Mutex
}

func (ch *ConversationsHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	var conversationsRequest map[string]any
	if err := json.Unmarshal(b, &conversationsRequest); err != nil {
		w.Write([]byte(err.Error())) //nolint:all
		w.WriteHeader(http.StatusBadRequest)
		return
	}
	...
	ch.mu.Lock()
	ch.ConversationsRequests = append(ch.ConversationsRequests, conversationsRequest)
	ch.mu.Unlock()
	...
	case "nixlv2":
		switch ch.Role {
		case RoleDecode:
			// Decode response for Conversations API
			rawResponse = `{"id":"conv_abc123","object":"conversation","created_at":1699000000,"status":"completed","output":[{"type":"message","id":"msg_abc123","status":"completed","role":"assistant","content":[{"type":"output_text","text":"Hello! How can I help you today?"}]}]}`
		case RolePrefill:
			// 1. Verify Prefill Request
			kvTransferParams, ok := conversationsRequest["kv_transfer_params"]
	...
	case "sglang":
		...
		rawResponse = `{"id":"conv_abc123","object":"conversation","created_at":1699000000,"status":"completed","output":[{"type":"message","id":"msg_abc123","status":"completed","role":"assistant","content":[{"type":"output_text","text":"Hello!"}]}]}`

	default:
		// Default case for unspecified connector (used for basic tests)
		rawResponse = `{"id":"conv_default","object":"conversation","created_at":1699000000,"status":"completed","output":[]}`
	}

	var conversationsResponse map[string]any
	if err := json.Unmarshal([]byte(rawResponse), &conversationsResponse); err != nil {
		w.WriteHeader(http.StatusBadRequest)
		w.Write([]byte(err.Error())) //nolint:all
		return
	}
	...
	ch.mu.Lock()
	ch.ConversationsResponses = append(ch.ConversationsResponses, conversationsResponse)
	ch.mu.Unlock()
