...
For GKE environments, if you are using **GKE Managed Prometheus** for metrics collection, set `inferenceExtension.monitoring.provider.name` to `gmp`. This will create the necessary `PodMonitoring` and RBAC resources for metrics collection.
...
| `provider.name`                                            | Name of the Inference Gateway implementation being used. Possible values: [`none`, `gke`, or `istio`]. Defaults to `none`. This is the gateway provider section, not a generic infrastructure provider.                                                                                                                                                                                       |
